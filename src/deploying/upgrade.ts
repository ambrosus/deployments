...
export interface UpgradeOptions<Factory> {
...
export function makeUpgrade(hre: HardhatRuntimeEnvironment) {
  ...
  async function upgrade<N extends ContractFactory>(
    {
      contractName,
      networkId,
      opts,
    ...
    if (!deployment.proxy) throw new Error(`Contract ${contractName} is not upgradeable`);
